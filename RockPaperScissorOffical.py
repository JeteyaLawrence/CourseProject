...
############### Putting Images to Buttons ####################################
class RockPaperScissorGame():
    def __init__(self):
        pygame.init()

        self.screen = pygame.display.set_mode((960, 640))
        pygame.display.set_caption("Rock Paper Scissor Smasher")
##################### Buttons to words #########################################
        self.bg = pygame.image.load("img/background.jpg")
        self.r_btn = pygame.image.load("img/r_button.png").convert_alpha() ##button 
        self.p_btn = pygame.image.load("img/p_button.png").convert_alpha()
        self.s_btn = pygame.image.load("img/s_button.png").convert_alpha()
        

############################ Images for words #####################################
        self.choose_rock = pygame.image.load("img/rock.png").convert_alpha() ##picture for button
...
####################################################################################
    def game_loop(self):
        ...
        rockpaperscissor_game = RockPaperScissorGame()
###################### Running the game ##################################################
        while run:
            ...
            for event in pygame.event.get():
                ...
                if event.type == pygame.MOUSEBUTTONDOWN:
                    if self.rock_btn.clicked(30) or self.paper_btn.clicked(340) or self.scissors_btn.clicked(640):
                        rockpaperscissor_game.image_reset()
                        rockpaperscissor_game.player()
                        rockpaperscissor_game.computer()

                        self.pl_score += rockpaperscissor_game.pl_score_cache()
                        self.pc_score += rockpaperscissor_game.pc_score_cache()
